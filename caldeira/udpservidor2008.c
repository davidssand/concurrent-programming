 ...
 * Funcionamento:
 *   O servidor recebe uma palavra do cliente, concatena algo, e envia de volta.
 */
...
int main(int argc, char *argv[])
{
	...
	if (argc < 2) {
		fprintf(stderr,"Uso: udpserver porta_escutada\n");
	...
	do{
		...
		printf("Adicionando 'ola tambem' na mensagem: %s\n", buffer);
		strcat( buffer, ", ola tambem\n");
